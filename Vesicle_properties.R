###  Plots for normalized vesicle properites ###

#set working directory + load data frame 

setwd("/Users/Ich/Nextcloud/AG Moissl-Eichinger_people/Viktoria/02_VESICLE_PAPER/Figures_Tables/Vesicle properties/normalization")
...
Protein <- ggplot(dataNORM, aes(x= Strain, y= normalized_protein, fill=Strain))+
  scale_fill_manual(values = Colors_Strains, name = "Strain") +
  geom_boxplot(data = dataNORM, aes(x = Strain, y = normalized_protein, fill = Strain), alpha = 1, width = 0.3, outlier.shape = NA) +
  labs (title = "Protein [nm/10^10 particles]", x = "Strains", y = "Protein [nm/10^10 particles]")

Protein + ylim(0, 55)

svglite::svglite(filename = "norm_Protein_AEV.svg", width = 10, height = 8)
print(Protein + ylim(0, 55))
dev.off()
